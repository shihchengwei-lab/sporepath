...
- 從 notes inbox 收手寫的 Markdown/txt 筆記，讓以前存下來的想法也能進同一條消化管線。
...
它現在會先啟動 ArcRift backend，再在背景啟動本地 source watcher 和離峰 digest queue worker，最後打開 Sporepath 小視窗。

正常只會留下 Sporepath 主視窗。ArcRift、source watcher 和 queue worker 會在背景隱藏執行；主視窗裡會顯示 ArcRift、Sources、Queue 的狀態燈。

- 本地 Codex / Claude / jsonl 來源會直接被 watcher 追蹤。檔案有變動時，Sporepath 會自動刷新 SQLite 記憶庫、digested notes、Obsidian vault 和 graph。
- 背景消化 queue worker 會隱藏啟動，收本地來源、ArcRift 和 notes inbox 的新 fragments，但只在設定的離峰時段消化。
- 舊筆記可以放在 `%USERPROFILE%\Documents\Sporepath Inbox`，支援 `.md`、`.markdown`、`.txt`。Sporepath 不會掃任意 ArcRift 資料夾來找筆記。
- 網頁聊天不再做背景自動抓取。你聊完 ChatGPT 或 Claude 網頁版之後，按 ArcRift popup 裡的 **Save Chat**。ArcRift 把聊天寫進 `ArcRift.db` 後，Sporepath 會把它當成另一個待整理來源丟進 queue，不會每 20 秒重建一次。
...
預設會用 `qwen3:1.7b`、`00:00-07:00`、每批 `5` 筆，用 `--source all` 自動收白名單裡的本地 Codex/Claude/jsonl 對話來源，並在旁邊的 `..\ArcRift\backend\ArcRift.db` 存在時一起收進 queue。它也會建立/使用 `%USERPROFILE%\Documents\Sporepath Inbox` 來收 Markdown/txt 舊筆記。處理出新 atoms 後，也會刷新 Obsidian vault 和 HTML graph。啟動前會先檢查 Ollama 和模型是否存在，避免模型沒裝時把 queue 項目標成錯誤。
...
- **Rate Suggestions**：只有 `Inspire` 回傳 `suggestion_id` 後才會出現，對每個 suggestion 按 👍、👎 或留空後提交結構化回饋。
- **Debug**：打開維修和設定面板。
...
如果要把 ArcRift 存下來的聊天排進同一條離峰 scout 消化管線：
...
正常的 `Sporepath.bat` 流程已經會啟動 queue worker，所以 ArcRift 存下來的聊天會由那條路徑撿起來。`watch-arcrift` 仍然保留成 debug/救援用的即時規則匯入，但不再是預設 launcher 的一部分。
...
桌面 app 會把 👍 記成 `useful`，把 👎 記成 `wrong`，把留空後送出的 suggestion 記成 `ignored`。Inspire run 和 feedback 也會寫入本地 `usage_events`，當作可稽核的使用軌跡。
...
目前要測已通過 validator 的 scout，可以直接跑：

```text
Run-Sporepath-Qwen17-Eval.bat
```

它會用 `qwen3:1.7b` 從白名單本地來源抽 35 筆、每個檔案最多 1 筆，跳過近似重複片段，並且每完成 1 筆就 checkpoint。輸出是 `eval\qwen17_eval.jsonl` 和 `eval\qwen17_eval.md`。接著它會自動跑 `eval-clean`，另外輸出 `eval\qwen17_eval.clean.jsonl` 和 `eval\qwen17_eval.clean.md`；請優先看 clean sheet 並補評分。
...
- `qwen3:1.7b` 已在一份 31 筆 clean eval 上通過 scout validator，但仍會製造 false positives，需要持續抽樣。
- 在這台機器上，`qwen3.5:4b` 比較慢，而且部分 scout prompt 會產生 invalid JSON；在它通過 `validate-scout` 前先視為實驗模型。
