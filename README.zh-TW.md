...
它現在會先啟動 ArcRift backend，再啟動最小化的本地 source watcher 和離峰 digest queue worker，最後打開 Sporepath 小視窗。

- 本地 Codex / Claude / jsonl 來源會直接被 watcher 追蹤。檔案有變動時，Sporepath 會自動刷新 SQLite 記憶庫、digested notes、Obsidian vault 和 graph。
- 背景消化 queue worker 會以最小化視窗啟動，收本地來源和 ArcRift 的新 fragments，但只在設定的離峰時段消化。
...
預設會用 `qwen3:1.7b`、`00:00-07:00`、每批 `5` 筆，用 `--source all` 自動收白名單裡的本地 Codex/Claude/jsonl 對話來源，並在旁邊的 `..\ArcRift\backend\ArcRift.db` 存在時一起收進 queue。處理出新 atoms 後，也會刷新 Obsidian vault 和 HTML graph。啟動前會先檢查 Ollama 和模型是否存在，避免模型沒裝時把 queue 項目標成錯誤。
