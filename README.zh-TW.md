...
預設會用 `qwen3:1.7b`、`00:00-07:00`、每批 `5` 筆，並用 `--source all` 自動收白名單裡的本地 Codex/Claude/jsonl 對話來源。處理出新 atoms 後，也會刷新 Obsidian vault 和 HTML graph。啟動前會先檢查 Ollama 和模型是否存在，避免模型沒裝時把 queue 項目標成錯誤。
...
正向 feedback 應該帶一段簡短 `--note`；`validate-inspire` 會要求大多數正向 feedback 有理由，避免空按 useful 就讓流程過關。Inspire run 和 feedback 也會寫入本地 `usage_events`，當作可稽核的使用軌跡。
...
- **靈感回饋**：`inspire` 產生的下一手，要有一定比例真的被你標成有用，且正向 feedback 要有簡短理由，否則這套流程只是好看。
